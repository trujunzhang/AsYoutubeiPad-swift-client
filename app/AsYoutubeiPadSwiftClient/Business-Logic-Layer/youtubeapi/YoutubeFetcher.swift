...
class YoutubeFetcher: NSObject {
    ...
    func search(q: NSString, completeHandler: ObjectHandler) {
        ...
        var query: GTLQueryYouTube = GTLQueryYouTube.queryForVideosListWithPart("id,snippet") as GTLQueryYouTube
        query.maxResults = 50
        query.type = "video"
        query.q = q
        ...
        service.executeQuery(query, completionHandler: {
            ...
            if (error == nil) {
                let result = resultList as GTLYouTubeVideoListResponse
                let array = result.items() as NSArray
                if (self._delegate != nil) {
                    self._delegate?.endFetchingUserSubscriptions(array)
                }
